...
function getInputSamplingSnippet(inInfo, outShapeInfo, usesPackedTextures) {
    ...
    if (inShape.length <= outShape.length) {
        if (usesPackedTextures) {
            if (broadcast_util_1.getBroadcastDims(inShape, outShape).length === 0) {
...
function getPackedSamplerAtOutputCoords(inputInfo, outShapeInfo) {
    ...
    var packedTexShape = tex_util.getPackedMatrixTextureShapeWidthHeight(outTexShape[1], outTexShape[0]).slice();
    var inShape = inputInfo.shapeInfo.logicalShape;
    var outShape = outShapeInfo.logicalShape;
    var broadcastDims = broadcast_util_1.getBroadcastDims(inShape, outShape);
    ...
    var packedInTexShape = tex_util.getPackedMatrixTextureShapeWidthHeight(inTexShape[1], inTexShape[0]).slice();
    var glsl = glsl_version_1.getGlslDifferences();
    if (util.arraysEqual(inTexShape, outTexShape) &&
        util.sizeFromShape(inShape) === util.sizeFromShape(outShape)) {
        return "\n      vec4 " + funcName + "() {\n        return " + glsl.texture2D + "(" + texName + ", resultUV);\n      }\n    ";
    }
    var output = "return " + glsl.texture2D + "(" + texName + ", uv)";
